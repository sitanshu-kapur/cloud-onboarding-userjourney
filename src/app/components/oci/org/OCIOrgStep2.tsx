...
const PERMISSIONS = [
  { key: "monitoring" as const, label: "Security Monitoring", description: "Inspect all resources across all tenancies, read audit logs and Cloud Guard findings", locked: true },
  { key: "remediation" as const, label: "Remediation", description: "Manage security findings and apply recommended remediations", locked: false },
  { key: "dataScanning" as const, label: "Data Scanning", description: "Read Object Storage buckets, database metadata", locked: false },
  { key: "workloadVMs" as const, label: "Workload — VMs & Compute", description: "Read compute instances, boot volumes, and images", locked: false },
  { key: "workloadContainers" as const, label: "Workload — Containers & OKE", description: "Read OKE clusters, container images, and Artifact Registry", locked: false },
];
...
export function OCIOrgStep2({ data, onChange, onNext, onBack }: Props) {
  const toggle = (key: keyof OCIOrgFormData["permissions"]) => {
    ...
    <div className="flex flex-col flex-1 min-h-0">
      <div className="flex-1 overflow-auto px-8 py-6">
        ...
        <p className="text-sm text-[#64748B] mb-6">
          These permissions apply across all tenancies in the organization via cross-tenancy policies.
        </p>

        <div className="max-w-xl flex flex-col gap-3">
          {/* Auto-sync toggle */}
          <div className={`flex items-start gap-4 p-4 rounded-xl border mb-1 transition-colors ${data.autoSyncTenancies ? "border-[#C74634] bg-[#FEF2EE]" : "border-[#E2E8F0] bg-white"}`}>
            <Toggle on={data.autoSyncTenancies} locked={false} onToggle={() => onChange({ ...data, autoSyncTenancies: !data.autoSyncTenancies })} />
            <div>
              <p className="text-sm font-medium text-[#1E293B]">Auto-sync new child tenancies</p>
              <p className="text-xs text-[#64748B] mt-0.5">
                New tenancies added to the OCI Organization are automatically detected and onboarded via OCI Event Service
              </p>
            </div>
          </div>
            ...
            <div key={key} className={`flex items-start gap-4 p-4 rounded-xl border transition-colors ${data.permissions[key] ? "border-[#C74634] bg-[#FEF2EE]" : "border-[#E2E8F0] bg-white"}`}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {locked && <span className="text-xs bg-[#FEF2EE] text-[#C74634] border border-[#FECACA] px-1.5 py-0.5 rounded-full">Required</span>}
                </div>
      ...
      <div className="flex items-center justify-between px-8 py-4 border-t border-[#E2E8F0] shrink-0">
        ...
        <button onClick={onNext} className="px-6 py-2 rounded-lg bg-[#C74634] hover:bg-[#A8372A] text-white text-sm transition-colors">Next</button>
      </div>
    </div>
  );
}
